...
class MeatsManager {
    constructor() {
        ...
        this.categoryId = 'meats'; // Changed from 'grains' to 'meats'
        this.init();
    }
    ...
    async init() {
        try {
            this.startClock();
            await this.loadMeatsProducts(); // Changed from loadGrainsProducts
    ...
    // Load all products from the meats category subcollection
    async loadMeatsProducts() { // Changed from loadGrainsProducts
        try {
            console.log("Loading meats products..."); // Changed from "Loading grains products..."
            
            // Access the products subcollection under the meats category
            ...
            if (querySnapshot.empty) {
                console.log("No meats products found"); // Changed from "No grains products found"
            } else {
            ...
            console.error('Error loading meats products:', error); // Changed from 'Error loading grains products'
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No meats products found</h3> <!-- Changed from "No grains products" -->
                            <p>Add some meats to your inventory to see them here.</p> <!-- Changed from "Add some grains" -->
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the meats category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from meats
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the meats subcollection
            ...
            // Remove from meats subcollection
            await deleteDoc(productRef);
            console.log('Product removed from meats:', product.name); // Changed from 'grains'
...
document.addEventListener('DOMContentLoaded', () => {
    window.meatsManager = new MeatsManager(); // Changed from grainsManager to meatsManager
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.meatsManager) { // Changed from grainsManager to meatsManager
        window.meatsManager.destroy();
    }
});