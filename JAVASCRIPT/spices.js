...
class SpicesManager {
    constructor() {
        ...
        this.categoryId = 'spices'; // Changed from 'grains' to 'spices'
        this.init();
    }
    ...
    async init() {
        try {
            this.startClock();
            await this.loadSpicesProducts(); // Changed from loadGrainsProducts
    ...
    // Load all products from the spices category subcollection
    async loadSpicesProducts() { // Changed from loadGrainsProducts
        try {
            console.log("Loading spices products..."); // Changed from "Loading grains products..."
            
            // Access the products subcollection under the spices category
            ...
            if (querySnapshot.empty) {
                console.log("No spices products found"); // Changed from "No grains products found"
            } else {
            ...
            console.error('Error loading spices products:', error); // Changed from 'Error loading grains products'
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            <div class="no-products-icon">🌶️</div> <!-- Changed from 🌾 to 🌶️ -->
                            <h3>No spices products found</h3> <!-- Changed from "No grains products" -->
                            <p>Add some spices to your inventory to see them here.</p> <!-- Changed from "Add some grains" -->
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the spices category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from spices
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the spices subcollection
            ...
            // Remove from spices subcollection
            await deleteDoc(productRef);
            console.log('Product removed from spices:', product.name); // Changed from 'grains'
...
document.addEventListener('DOMContentLoaded', () => {
    window.spicesManager = new SpicesManager(); // Changed from grainsManager to spicesManager
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.spicesManager) { // Changed from grainsManager to spicesManager
        window.spicesManager.destroy();
    }
});