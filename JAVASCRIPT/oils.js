...
class OilsManager {
    constructor() {
        ...
        this.categoryId = 'oils'; // The category document ID
        this.init();
    }
    ...
    // Load all products from the oils category subcollection
    async loadOilsProducts() {
        try {
            console.log("Loading oils products..."); // Debug log
            
            // Access the products subcollection under the oils category
            ...
            if (querySnapshot.empty) {
                console.log("No oils products found"); // Debug log
            } else {
            ...
            console.error('Error loading oils products:', error);
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No oils products found</h3>
                            <p>Add some oils to your inventory to see them here.</p>
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the oils category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from oils
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the oils subcollection
            ...
            // Remove from oils subcollection
            await deleteDoc(productRef);
            console.log('Product removed from oils:', product.name);
...
document.addEventListener('DOMContentLoaded', () => {
    window.oilsManager = new OilsManager();
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.oilsManager) {
        window.oilsManager.destroy();
    }
});