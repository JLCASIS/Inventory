...
class SaucesManager {
    constructor() {
        ...
        this.categoryId = 'sauces'; // Changed from 'grains' to 'sauces'
        this.init();
    }
    ...
    async init() {
        try {
            this.startClock();
            await this.loadSaucesProducts(); // Changed from loadGrainsProducts
    ...
    // Load all products from the sauces category subcollection
    async loadSaucesProducts() { // Changed from loadGrainsProducts
        try {
            console.log("Loading sauces products..."); // Changed from "Loading grains products..."
            
            // Access the products subcollection under the sauces category
            ...
            if (querySnapshot.empty) {
                console.log("No sauces products found"); // Changed from "No grains products found"
            } else {
            ...
            console.error('Error loading sauces products:', error); // Changed from 'Error loading grains products'
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No sauces products found</h3> <!-- Changed from grains to sauces -->
                            <p>Add some sauces to your inventory to see them here.</p> <!-- Changed from grains to sauces -->
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the sauces category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from sauces
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the sauces subcollection
            ...
            // Remove from sauces subcollection
            await deleteDoc(productRef);
            console.log('Product removed from sauces:', product.name); // Changed from grains to sauces
...
document.addEventListener('DOMContentLoaded', () => {
    window.saucesManager = new SaucesManager(); // Changed from grainsManager to saucesManager
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.saucesManager) { // Changed from grainsManager to saucesManager
        window.saucesManager.destroy();
    }
});