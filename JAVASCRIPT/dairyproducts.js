...
class DairyManager {
    constructor() {
        ...
        this.categoryId = 'dairy'; // Changed from 'grains' to 'dairy'
        this.init();
    }
    ...
    async init() {
        try {
            this.startClock();
            await this.loadDairyProducts(); // Changed from loadGrainsProducts
    ...
    // Load all products from the dairy category subcollection
    async loadDairyProducts() { // Changed from loadGrainsProducts
        try {
            console.log("Loading dairy products..."); // Changed from "Loading grains products..."
            
            // Access the products subcollection under the dairy category
            ...
            if (querySnapshot.empty) {
                console.log("No dairy products found"); // Changed from "No grains products found"
            } else {
            ...
            console.error('Error loading dairy products:', error); // Changed from 'Error loading grains products'
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No dairy products found</h3> <!-- Changed from "No grains products found" -->
                            <p>Add some dairy to your inventory to see them here.</p> <!-- Changed from "Add some grains" -->
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the dairy category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from dairy
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the dairy subcollection
            ...
            // Remove from dairy subcollection
            await deleteDoc(productRef);
            console.log('Product removed from dairy:', product.name); // Changed from 'grains' to 'dairy'
...
document.addEventListener('DOMContentLoaded', () => {
    window.dairyManager = new DairyManager(); // Changed from grainsManager to dairyManager
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.dairyManager) { // Changed from grainsManager to dairyManager
        window.dairyManager.destroy();
    }
});