...
class GreensManager {
    constructor() {
        ...
        this.categoryId = 'greens'; // The category document ID
        this.init();
    }
    ...
    // Load all products from the greens category subcollection
    async loadGreensProducts() {
        try {
            console.log("Loading greens products..."); // Debug log
            
            // Access the products subcollection under the greens category
            ...
            if (querySnapshot.empty) {
                console.log("No greens products found"); // Debug log
            } else {
            ...
            console.error('Error loading greens products:', error);
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No greens products found</h3>
                            <p>Add some greens to your inventory to see them here.</p>
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the greens category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from greens
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the greens subcollection
            ...
            // Remove from greens subcollection
            await deleteDoc(productRef);
            console.log('Product removed from greens:', product.name);
...
document.addEventListener('DOMContentLoaded', () => {
    window.greensManager = new GreensManager();
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.greensManager) {
        window.greensManager.destroy();
    }
});