...
class EggsManager {
    constructor() {
        ...
        this.categoryId = 'eggs'; // The category document ID
        this.init();
    }
    ...
    // Load all products from the eggs category subcollection
    async loadEggsProducts() {
        try {
            console.log("Loading eggs products..."); // Debug log
            
            // Access the products subcollection under the eggs category
            ...
            if (querySnapshot.empty) {
                console.log("No eggs products found"); // Debug log
            } else {
            ...
            console.error('Error loading eggs products:', error);
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No eggs products found</h3>
                            <p>Add some eggs to your inventory to see them here.</p>
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the eggs category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from eggs
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the eggs subcollection
            ...
            // Remove from eggs subcollection
            await deleteDoc(productRef);
            console.log('Product removed from eggs:', product.name);
...
document.addEventListener('DOMContentLoaded', () => {
    window.eggsManager = new EggsManager();
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.eggsManager) {
        window.eggsManager.destroy();
    }
});