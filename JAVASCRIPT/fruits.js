...
class FruitsManager {
    constructor() {
        ...
        this.categoryId = 'fruits'; // Changed from 'grains' to 'fruits'
        this.init();
    }
    ...
    async init() {
        try {
            this.startClock();
            await this.loadFruitsProducts(); // Changed from loadGrainsProducts
    ...
    // Load all products from the fruits category subcollection
    async loadFruitsProducts() { // Changed from loadGrainsProducts
        try {
            console.log("Loading fruits products..."); // Changed from "Loading grains products..."
            
            // Access the products subcollection under the fruits category
            ...
            if (querySnapshot.empty) {
                console.log("No fruits products found"); // Changed from "No grains products found"
            } else {
            ...
            console.error('Error loading fruits products:', error); // Changed from 'Error loading grains products'
        ...
        if (querySnapshot.empty) {
            this.productsContainer.innerHTML = `
                        <div class="no-products">
                            ...
                            <h3>No fruits products found</h3> <!-- Changed from "No grains products found" -->
                            <p>Add some fruits to your inventory to see them here.</p> <!-- Changed from "Add some grains..." -->
                        </div>
                    `;
            return;
        }
    ...
    async checkAndMoveExpiringProducts() {
        ...
        try {
            // Access products from the fruits category subcollection
            const productsRef = collection(db, 'categories', this.categoryId, 'products');
    ...
    // Move product to expiry collection and remove from fruits
    async moveToExpiry(product, productId, daysLeft) {
        try {
            // Reference to the product in the fruits subcollection
            ...
            // Remove from fruits subcollection
            await deleteDoc(productRef);
            console.log('Product removed from fruits:', product.name); // Changed from 'grains' to 'fruits'
...
document.addEventListener('DOMContentLoaded', () => {
    window.fruitsManager = new FruitsManager(); // Changed from grainsManager to fruitsManager
});

// Cleanup on page unload
window.addEventListener('beforeunload', () => {
    if (window.fruitsManager) { // Changed from grainsManager to fruitsManager
        window.fruitsManager.destroy();
    }
});